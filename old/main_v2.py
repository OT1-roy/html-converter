...
def clean_html_for_llm(soup_tag: element.Tag) -> str:
    """
    Surgically extracts only semantically meaningful tags (p, h*, ul, ol, etc.)
    to create a clean HTML string, ideal for LLM training data.
    This avoids converting structural divs and other web components.
    """
    if not soup_tag:
        return ""
        
    # Find all tags that represent actual content.
    content_tags = soup_tag.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'ul', 'ol', 'li', 'blockquote'])
    
    # Reconstruct the HTML from only these tags.
...
def process_html_files(input_dir: str, output_dir: str) -> None:
    ...
    # Get the base name of the input directory for the output filename prefix.
    input_folder_name = os.path.basename(os.path.normpath(input_dir))
    output_filename_template = f"{input_folder_name}_output_{{}}.md"
            ...
            for filename in pbar:
                try:
                    ...
                    if candidates:
                        ...
                        if candidates[best_candidate_element] < MIN_CONTENT_SCORE:
                    ...
                    if html_to_process:
                        # NEW: Run the surgical cleaning step before conversion
                        clean_html = clean_html_for_llm(html_to_process)
                        markdown_text = convert_html_to_markdown_pandoc(clean_html)
